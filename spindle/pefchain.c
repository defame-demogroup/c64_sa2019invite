...
static void create_blank_effect(struct effect *e) {
	e->header.flags = EF_SAFE_IO | EF_BLANK;
...
static int preferred_loading_slot(int later, int earlier) {
	...
	if((effect[earlier].header.flags & EF_BLANK)
	&& !(effect[later].header.flags & EF_BLANK)) return later;
	return earlier;
}
		...
		!= ((effect[i].driver_org + pos - 1) & 0xff00)) {
			...
			if(page == 0xd0) {
				...
				errx(1, "Driver for effect '%s' (%d bytes at $%04x) collides with used/inherited page %02x",
					...
					effect[i].driver_org,
					page);
			} else if(i && effect[i - 1].header.pageflags[page] & (PF_LOADED | PF_USED | PF_INHERIT)) {
				errx(1, "Driver for effect '%s' (%d bytes at $%04x) collides with previous effect '%s' at $%02x00",
					effect[i].filename,
					effect[i].driver_size,
					effect[i].driver_org,
					effect[i - 1].filename,
					page);
			} else if(i < neffect - 1 && effect[i + 1].header.pageflags[page] & (PF_LOADED | PF_USED | PF_INHERIT)) {
				errx(1, "Driver for effect '%s' (%d bytes at $%04x) collides with next effect '%s' at $%02x00",
					effect[i].filename,
					effect[i].driver_size,
					effect[i].driver_org,
					effect[i + 1].filename,
...
static void make_dirart(uint8_t *dirart, char *fname) {
	...
	if(fname) {
		f = fopen(fname, "r");
		if(!f) err(1, "make_dirart: fopen: %s", fname);
		while(fgets(buf, sizeof(buf), f)) {
			if(strlen(buf) && buf[strlen(buf) - 1] == '\n') {
				buf[strlen(buf) - 1] = 0;
			}
...
static void usage(char *prgname) {
	...
	fprintf(stderr, "\n");
	fprintf(stderr, "  -f --fast-serial  Reduce safety margin in serial transfer routine.\n");
	fprintf(stderr, "  -E --errors       Simulate read/transfer errors with a given probability.\n");
	fprintf(stderr, "\n");
...
int main(int argc, char **argv) {
	...
	int opt, dir_entry = 0, visualisation_width = 4, fastserial = 0, err_prob = 0;
	...
	do {
		opt = getopt_long(argc, argv, "?hVvo:wQa:d:fE:t:i:m:n:", longopts, 0);
		switch(opt) {
			case 0:
			case '?':
			case 'h':

	...
	int err_prob)
{
	...
	if(err_prob) {
		if(fastserial) errx(1,
			"Can't combine error injection with fast serial transfer.");
		stage1 = data_stage1err;
		stage1size = sizeof(data_stage1err);
		drivecode = data_drivecodeerr;
	} else if(fastserial) {
		stage1 = data_stage1fast;
		stage1size = sizeof(data_stage1fast);
	...
	if(stage1size > 254 * 3) {
		...
		SECTOR(18, 3)[0] = 0;
		SECTOR(18, 3)[1] = stage1size - 3 * 254 + 1;
		memcpy(
			SECTOR(18, 3) + 2,
			stage1 + 3 * 254,
			stage1size - 3 * 254);
